...
这句话其实解释了很多：
1.为什么默认情况下block在栈上？因为block是一个**结构体类型的局部变量**，而局部变量正是在栈上
...
| stack  | 捕获了局部变量 or 捕获成员变量也算；且没有被strong/copy关键字描述   | block被copy到堆上 | 不会增加捕获对象的引用计数(大多数文章都是这样写的，但是在arc下通过CFGetRetainCount结论不一致) | 栈区 | 超过作用域系统回收|
...
何为捕获？
即用clang重写后**block对应的结构中，是否有访问的变量；是值的方式还是引用的形式**
...
3.__block解决循环引用
也可以，但是必须要调用block,且要__block变量置为nil
```
