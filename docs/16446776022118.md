...
它是递归锁(`os_unfair_lock`)，代码块的开始和结束分别调用`objc_sync_enter`和 `objc_sync_exit`。
...
### 加锁分析
1. 首先分析锁对象是否在线程的存储空间中(thread local storage)中，如果在的话，判断是加锁还是解锁，然后操作lockCount++ / lockCount--，然后存储到TLS中
2. 如果不在，则查询SyncCache, 所做的事和上一步一样。
3. 如果前面都没有找到，则从链表开始遍历，找到加锁的元素，将threadCount++ 
...
2.SyncData 是加锁对象的结构体包装，每一个解锁对象，都有一把锁，用于synchronized代码块中的加解锁
```
...
## 区别
概念上：
进程是运行中的程序，它是操作系统资源分配资源的基本单位。每个进程有自己的地址空间：以Segment划分，可以分为代码段、数据段、堆区、共享库区、栈区等；进程还管理着页表，它表示逻辑地址到物理地址的映射；进程也管理着VMA，它代表虚拟地址到可执行文件的映射
此外进程还管理打开的文件和IO设备
...
1.提升效率
2.线程的创建、销毁、切换等更快
3.同一进程内的线程共享进程的内存和资源，那么在线程之间切换的时候，不需要经过内核，这样效率就提供了
...
3.从用户态到内核态的切换是有代价的，因此内核线程的切换比用户线程的切换代价大；内核线程表存在操作系统的表格空间或堆栈空间，因此内核线程数量有限、扩展性不如用户线程
